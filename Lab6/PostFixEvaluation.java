...
public class PostFixEvaluation {
    ...
    {
        Stack<Integer> stek = new ArrayStack<>(n);
        StringBuilder broj = new StringBuilder();

        for(int i = 0; i<n; i++) {

            if(izraz[i] == ' ' && broj.length() > 0) {
                stek.push(Integer.parseInt(broj.toString()));
                broj.setLength(0);
            }
            ...
            if(izraz[i] == '+') {
                int prvBroj = stek.pop();
                int vtorBroj = stek.pop();
                stek.push(prvBroj + vtorBroj);
            }

            if(izraz[i] == '-') {
                int prvBroj = stek.pop();
                int vtorBroj = stek.pop();
                stek.push(vtorBroj - prvBroj);
            }
