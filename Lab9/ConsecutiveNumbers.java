...
public class ConsecutiveNumbers {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // vasiot kod ovde
        BNode<Integer> tekovenJazol = null;
        BNode<Integer> prethodenJazol = null;
        Stack<BNode<Integer>> stek = new Stack<>();
        stek.push(tree.root);

        do {
            prethodenJazol = tekovenJazol;
            tekovenJazol = stek.pop();
            int razlika = 0;

            if (prethodenJazol != null) {
                razlika = tekovenJazol.info - prethodenJazol.info;
                if (razlika != 2) {
                    System.out.println(false);
                    return;
                }
            }
            if (tekovenJazol.right != null) {
                stek.push(tekovenJazol.right);
            }
            if (tekovenJazol.left != null) {
                stek.push(tekovenJazol.left);
            }
        } while (stek.size() != 0);
