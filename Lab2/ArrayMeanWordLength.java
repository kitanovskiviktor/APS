...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        ...
        for(int i=0; i<arr.getSize(); i++) {
            suma = suma + arr.get(i).length();
        }
        float prosek = suma / arr.getSize();
        String zbor = arr.get(0);
        double najmalaRazlika = Math.abs(zbor.length() - prosek);
        for(int i = 0; i<arr.getSize(); i++){

            String momentalenZbor = arr.get(i);
            float razlika = Math.abs(momentalenZbor.length() - prosek);
            if(razlika < najmalaRazlika || (razlika == najmalaRazlika && momentalenZbor.length() > zbor.length())) {
                najmalaRazlika = razlika;
                zbor = arr.get(i);
            }
        }
        return zbor;
